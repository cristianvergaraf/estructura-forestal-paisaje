...
# la rescalada, para comprar los resultados.
...
fa.parallel(pl[,3:18], fm = "ml", fa = "fa", n.iter = 500, ylab = "Eigenvalue")
...
EFA_model <- fa(pl[,3:18],3, rotate = "varimax", fm = "ml")
EFA_model1 <- fa(pl_norm[,3:18],3, rotate = "oblimin", fm = "ml") 
EFA_model2 <- fa(pl_rescale[,3:18],3, rotate = "Promax", fm = "ml") 

# Podemos acceder a un resumen de los resultados con la función print y el nombre d l
# modelo

print(EFA_model)

## Esto es un poco caótico, mejor es acceder a los resultados de forma individual.
...
### Los resultados entre las 3 bases de datos no varian mucho en los loadings. 
...
## expliquen el mayor de variabilidad y que tengan la más fácil interpretación
